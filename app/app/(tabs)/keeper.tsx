...
import {
  ...
  KeyboardAvoidingView, Platform, PanResponder, BackHandler,
} from 'react-native';
...
import { useStore, MODE_ACCENTS, getPersonaEmoji, getPersonaName } from '../../lib/store';
import SettingsModal from '../../components/SettingsModal';

const KEEPER_ACCENT = MODE_ACCENTS.keeper;
...
function buildMarkdownStyles(theme: import('../../lib/theme').Theme) {
  ...
  return {
    ...
    blockquote:  { backgroundColor: theme.surface, borderLeftColor: KEEPER_ACCENT, borderLeftWidth: 3, paddingHorizontal: 14, paddingVertical: 2, marginVertical: 8, borderRadius: 4 },
    code_inline: { fontFamily: mono, fontSize: 13, backgroundColor: theme.surfaceAlt, color: KEEPER_ACCENT, paddingHorizontal: 5, paddingVertical: 2, borderRadius: 4 },
    ...
    link:        { color: KEEPER_ACCENT, textDecorationLine: 'underline' as const },
...
export default function WikiScreen() {
  ...
  // Ingest result
  const [ingestResult, setIngestResult] = useState<{ todos_added: number; pages_created: string[]; pages_updated: string[]; project_flag: string | null } | null>(null);
  ...
  }, [fetchProjects, fetchJournal, fetchRawStatus]);
  ...
  const fetchTree = useCallback(async (isRefresh = false, overridePath?: string) => {
    ...
    try {
      const res = await apiFetch(`/wiki/tree?path=${encodeURIComponent(overridePath ?? currentPath)}`);
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    fetchRawStatus();
  }, [fetchTree, fetchProjects, fetchJournal, fetchRawStatus, topFolder, isSubfolder]);
  ...
  const ingestFileFromRaw = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json() as { ok?: boolean; pages_created?: string[]; pages_updated?: string[]; error?: string };
    ...
    try {
      if (captureProcessNow) {
        ...
        const data = await res.json() as {
          ...
          pages_updated?: string[]; project_flag?: string | null; error?: string;
        };
  ...
  const runReindex = useCallback(async () => {
    setReindexing(true);
    try {
      const res = await apiFetch('/knowledge/reindex', { method: 'POST' });
      const data = await res.json() as { ok?: boolean; indexed?: string[]; skipped?: string[]; removed?: string[]; errors?: string[]; error?: string };
      if (!res.ok) throw new Error(data.error ?? 'Reindex failed');
      const n = data.indexed?.length ?? 0;
      flashToolsMsg(n === 0 ? 'Index up to date' : `Indexed ${n} file${n === 1 ? '' : 's'}`);
    } catch (err) {
      flashToolsMsg(err instanceof Error ? err.message : 'Reindex failed');
    } finally {
      setReindexing(false);
    }
  }, [flashToolsMsg]);
  ...
  const runIngestRaw = useCallback(async () => {
    ...
    setToolsMsg(null);
    try {
      const res = await apiFetch('/wiki/ingest-raw', { method: 'POST' });
      if (!res.ok) {
        const body = await res.json().catch(() => ({})) as { error?: string };
        throw new Error(body.error ?? `Server error ${res.status}`);
      }
      const data = await res.json();
      const msg = `Ingest raw: ${data.processed?.length ?? 0} processed, ${data.skipped?.length ?? 0} skipped${data.errors?.length ? `, ${data.errors.length} errors` : ''}`;
      setToolsMsg(msg);
      if ((data.processed?.length ?? 0) > 0 && topFolder === 'wiki') fetchTree(false, 'wiki');
    } catch (err) {
  ...
  const renderEntry = ({ item }: { item: TreeEntry }) => {
    ...
    return (
      ...
      >
        ...
        {isPendingRaw ? (
          <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: KEEPER_ACCENT, marginRight: 12 }} />
        ) : null}
      </TouchableOpacity>
    );
  };
    ...
    <View style={[styles.root, { paddingTop: insets.top, backgroundColor: theme.bg }]} {...swipe.panHandlers}>
      ...
      <View style={[styles.personaHeader, { borderBottomColor: theme.border }]}>
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Text style={[styles.personaTitle, { color: KEEPER_ACCENT }]}>
            {getPersonaEmoji('keeper', personas)} {getPersonaName('keeper', personas)}
          </Text>
        </TouchableOpacity>
      ...
      <View style={[styles.headerRow, { backgroundColor: theme.surface, borderBottomColor: theme.border }]}>
        ...
        >
          {toolsBusy ? (
            <ActivityIndicator size="small" color={KEEPER_ACCENT} />
          ) : (
      ...
      <View style={[styles.contentArea, {
        backgroundColor: isSubfolder ? KEEPER_ACCENT + '07' :
          topFolder === 'projects' ? KEEPER_ACCENT + '07' :
      ...
      {topFolder === 'projects' && pathStack.length === 1 ? (
        ...
        <View style={styles.flex}>
          {/* Pinned wiki meta-files */}
          <TouchableOpacity
            style={[styles.row, styles.wikiPinnedRow, { borderBottomColor: theme.border, backgroundColor: KEEPER_ACCENT + '10' }]}
            onPress={() => openFile({ name: '_index.md', path: 'wiki/_index.md', size: 0, mtime: '', isDir: false })}
            activeOpacity={0.7}
          >
            <Text style={styles.rowIcon}>📋</Text>
            <View style={styles.rowBody}>
              <Text style={[styles.rowName, { color: KEEPER_ACCENT }]}>Index</Text>
              <Text style={[styles.rowMeta, { color: theme.textDim }]}>All wiki pages — catalog</Text>
            </View>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.row, styles.wikiPinnedRow, { borderBottomColor: theme.border, backgroundColor: KEEPER_ACCENT + '10' }]}
          ...
          >
            <Text style={styles.rowIcon}>📜</Text>
            <View style={styles.rowBody}>
              <Text style={[styles.rowName, { color: KEEPER_ACCENT }]}>Log</Text>
          ...
          {loading ? (
            ...
            <FlatList
              data={entries.filter(e => e.name !== '_index.md' && e.name !== 'log.md')}
              keyExtractor={(item) => item.path}
              renderItem={renderEntry}
              refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetchTree(true)} tintColor={theme.textDim} />}
              ListEmptyComponent={<View style={styles.centered}><Text style={[styles.emptyText, { color: theme.textDim }]}>No wiki pages yet — capture notes in Raw, then process them here</Text></View>}
              contentContainerStyle={entries.filter(e => e.name !== '_index.md' && e.name !== 'log.md').length === 0 ? styles.emptyContainer : styles.listPad}
            />
          )}
        </View>
      ) : (
      ...
      <View style={styles.fabRow}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <Text style={[styles.fabLabel, { color: KEEPER_ACCENT }]}>? Ask</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <View style={[styles.sheet, { backgroundColor: theme.surface, borderColor: theme.border, paddingBottom: insets.bottom + 12 }]}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text style={[styles.sheetRowIcon, { color: KEEPER_ACCENT }]}>🧠</Text>
          ...
          <TouchableOpacity
            style={[styles.sheetRow, { borderBottomColor: theme.border }]}
            onPress={() => { setToolsSheetVisible(false); setIngestModalVisible(true); }}
            activeOpacity={0.7}
          >
            <Text style={[styles.sheetRowIcon, { color: KEEPER_ACCENT }]}>⚙</Text>
            <View style={styles.sheetRowBody}>
              <Text style={[styles.sheetRowTitle, { color: theme.text }]}>Capture</Text>
              <Text style={[styles.sheetRowDesc, { color: theme.textDim }]}>Write a note — save fast or process to wiki</Text>
            </View>
          </TouchableOpacity>
        ...
        <View style={[styles.modalRoot, { backgroundColor: theme.bg, paddingTop: insets.top }]}>
          <View style={[styles.modalHeader, { borderBottomColor: theme.border }]}>
            ...
            {fileDetail?.path.startsWith('raw/') ? (
              <TouchableOpacity onPress={ingestFileFromRaw} style={styles.modalHeaderBtn} disabled={ingestingFile || fileLoading}>
                <Text style={[styles.modalHeaderBtnLabel, { color: KEEPER_ACCENT }]}>
                  {ingestingFile ? '…' : '↑ Ingest'}
                </Text>
              </TouchableOpacity>
            ) : fileDetail && !fileDetail.path.startsWith('wiki/') ? (
              <TouchableOpacity onPress={editMode ? saveFile : () => setEditMode(true)} style={styles.modalHeaderBtn} disabled={saving || fileLoading}>
                <Text style={[styles.modalHeaderBtnLabel, { color: theme.text }]}>{editMode ? (saving ? '…' : 'Save') : 'Edit'}</Text>
              </TouchableOpacity>
            ) : (
      ...
      <Modal visible={ingestModalVisible} animationType="slide" presentationStyle="pageSheet" onRequestClose={closeIngestModal}>
        <KeyboardAvoidingView style={[styles.modalRoot, { backgroundColor: theme.bg }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={[styles.modalHeader, { borderBottomColor: theme.border }]}>
            <TouchableOpacity onPress={closeIngestModal} style={styles.modalHeaderBtn}>
              <Text style={[styles.modalHeaderBtnLabel, { color: theme.text }]}>✕</Text>
            </TouchableOpacity>
            <Text style={[styles.modalTitle, { color: theme.text }]}>Capture</Text>
            <View style={styles.modalHeaderBtn} />
          </View>
          <TextInput
            style={[styles.ingestInput, { color: theme.text, backgroundColor: theme.inputBg }]}
            value={ingestText}
          ...
          >
            <View style={{
              ...
              borderColor: captureProcessNow ? KEEPER_ACCENT : theme.border,
              backgroundColor: captureProcessNow ? KEEPER_ACCENT : 'transparent',
              marginRight: 10, alignItems: 'center', justifyContent: 'center',
            }}>
              {captureProcessNow ? <Text style={{ color: '#000', fontSize: 13, fontWeight: '700' }}>✓</Text> : null}
            </View>
            <Text style={{ color: theme.textDim, fontSize: 14 }}>Process now → wiki</Text>
          </TouchableOpacity>
          {ingestError ? <Text style={[styles.saveError, { color: '#FF6135' }]}>{ingestError}</Text> : null}
          <TouchableOpacity
            style={[styles.ingestSaveBtn, { backgroundColor: theme.surface, borderColor: theme.border }, (!ingestText.trim() || ingestSaving) && styles.ingestSaveBtnDisabled]}
            onPress={submitCapture}
            disabled={!ingestText.trim() || ingestSaving}
            activeOpacity={0.8}
          >
            <Text style={[styles.ingestSaveBtnLabel, { color: theme.text }]}>{ingestSaving ? 'Saving…' : captureProcessNow ? 'Save & Process' : 'Save to Raw'}</Text>
          </TouchableOpacity>
        </KeyboardAvoidingView>
      </Modal>
      ...
      <Modal visible={askModalVisible} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setAskModalVisible(false)}>
        <KeyboardAvoidingView style={[styles.modalRoot, { backgroundColor: theme.bg }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={[styles.modalHeader, { borderBottomColor: theme.border }]}>
            ...
            <Text style={[styles.modalTitle, { color: theme.text }]}>Ask Keeper</Text>
            <TouchableOpacity onPress={runAsk} style={styles.modalHeaderBtn} disabled={askLoading || !askQuestion.trim()}>
              <Text style={[styles.modalHeaderBtnLabel, { color: askQuestion.trim() ? KEEPER_ACCENT : theme.textFaint }]}>
                {askLoading ? '…' : 'Ask'}
              </Text>
            </TouchableOpacity>
          </View>
          ...
          {/* Filter chips */}
          <View style={styles.askFilterRow}>
            {(['all', 'wiki', 'projects', 'journal'] as const).map(f => (
              <TouchableOpacity
                key={f}
                onPress={() => setAskFilter(f)}
                style={[styles.askFilterChip, askFilter === f && { backgroundColor: KEEPER_ACCENT }]}
                activeOpacity={0.7}
              >
          ...
          <TextInput
            style={[styles.askInput, { color: theme.text, backgroundColor: theme.inputBg }]}
            value={askQuestion}
          ...
          <ScrollView style={styles.flex} keyboardShouldPersistTaps="handled">
            {askLoading ? (
              ...
              <Text style={[styles.toolsMsgText, { color: theme.textDim, padding: 20 }]}>{askError}</Text>
            ) : askSearched && !askAnswer && askResults.length === 0 ? (
              <Text style={[styles.emptyText, { color: theme.textDim, padding: 20 }]}>No matches found. Try adding more notes to your wiki.</Text>
            ) : !askSearched ? (
              <Text style={[styles.emptyText, { color: theme.textDim, padding: 20 }]}>Ask a question to search your knowledge base.</Text>
            ) : (
              <>
                {/* Synthesised answer */}
                {askAnswer ? (
                  <View style={[styles.askAnswerBlock, { backgroundColor: theme.surface, borderLeftColor: KEEPER_ACCENT }]}>
                    <Text style={[styles.askAnswerText, { color: theme.text }]}>{askAnswer}</Text>
                  </View>
                ) : null}
                ...
                {askResults.length > 0 ? (
                  <>
                    <TouchableOpacity
                      style={[styles.askSourcesToggle, { borderBottomColor: theme.border }]}
                    ...
                    {askSourcesExpanded && askResults.map((chunk, i) => (
                      <View key={i} style={[styles.askChunk, { borderBottomColor: theme.border }]}>
                        <Text style={[styles.askChunkPath, { color: KEEPER_ACCENT }]}>{chunk.path}{chunk.heading ? ` › ${chunk.heading}` : ''}</Text>
                ...
                {/* Save to wiki */}
                {askSearched && !askLoading && (askAnswer || askResults.length > 0) && (
                  <TouchableOpacity
                    style={[styles.saveToWikiBtn, { backgroundColor: KEEPER_ACCENT + '18', borderColor: KEEPER_ACCENT + '40' }]}
                    onPress={async () => {
                      try {
                    ...
                    activeOpacity={0.8}
                  >
                    <Text style={[styles.saveToWikiLabel, { color: KEEPER_ACCENT }]}>
          ...
          {ingestResult && (
            <>
              ...
              {ingestResult.project_flag && (
                <Text style={[styles.ingestResultNudge, { color: KEEPER_ACCENT }]}>
                  Sounds like it relates to "{ingestResult.project_flag}" — bring it up with Mentor or Shapeshifter.
                </Text>
              )}
              {ingestResult.todos_added === 0 && ingestResult.pages_created.length === 0 && ingestResult.pages_updated.length === 0 && (
                <Text style={[styles.ingestResultLine, { color: theme.textDim }]}>Nothing new to file.</Text>
              )}
            </>
          )}
        ...
        <View style={[styles.modalRoot, { backgroundColor: theme.bg }]}>
          ...
          <ScrollView style={styles.flex}>
            {dupesLoading ? (
              ...
              dupes.map((d, i) => (
                <View key={i} style={[styles.askChunk, { borderBottomColor: theme.border }]}>
                  <Text style={[styles.askChunkPath, { color: KEEPER_ACCENT }]}>{Math.round(d.similarity * 100)}% similar</Text>
